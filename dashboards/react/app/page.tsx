...
export default function Home() {
  ...
  const [view, setView] = useState<"overview" | "drivers" | "races" | "constructors">("overview");
  ...
  return <main className="min-h-screen bg-[#07111f] text-[#f7f9fc]">
    ...
    <section className="mx-auto max-w-7xl px-5 py-8 md:px-10">
      ...
      {status === "ready" && <><nav aria-label="Dashboard views" className="mb-7 flex flex-wrap gap-2"><ViewButton active={view === "overview"} onClick={() => setView("overview")}>Overview</ViewButton><ViewButton active={view === "drivers"} onClick={() => setView("drivers")}>Driver comparison</ViewButton><ViewButton active={view === "races"} onClick={() => setView("races")}>Race analysis</ViewButton><ViewButton active={view === "constructors"} onClick={() => setView("constructors")}>Constructor detail</ViewButton></nav><div className={view === "overview" ? "" : "hidden"}><div className="mb-7 flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm text-[#a8b5c7]">Season {season} · through round {round}</p><h2 className="mt-1 text-3xl font-black tracking-tight">At a glance</h2></div><p className="max-w-md text-right text-sm text-[#a8b5c7]">Data is a Snowflake MARTS snapshot, not live timing.</p></div>
